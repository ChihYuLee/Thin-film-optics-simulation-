...
% Please change the layer thickness other than PCM and ITO at block 6: d=1e-9*[thickPCM,30,thickITO,100,1000]
% thickPCM=tl;
tl_min=10;
tl_max=20;
tl_step=1;
b= ((tl_max- tl_min)/tl_step)+1; 

% thickITO=dl;
dl_min=50;
dl_max=150;
dl_step=10;
a=((dl_max- dl_min)/dl_step)+1;

% d=1e-9*[thickPCM,Ag,thickITO,Ag,1000];
...
for j=1:b;
 ...
 scatter(dl(i),tl(j),850,xyzAMO/max(xyzAMO),'filled','s');
 axis tight;
 g=gca;
 g.XRuler.TickLabelGapOffset = 15;
 g.YRuler.TickLabelGapOffset = 15;
 xlabel('ITO (nm)');
 ylabel('PCM (nm)');
 end
 end
...
for j=1:b;
 ...
 scatter(dl(i),tl(j),850,xyzCRY/max(xyzCRY),'filled','s');
 axis tight;
 g=gca;
 g.XRuler.TickLabelGapOffset = 15;
 g.YRuler.TickLabelGapOffset = 15;
